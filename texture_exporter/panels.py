...
class TEXTURE_EXPORTER_PT_main_panel(Panel):
    ...
    def draw(self, context):
        ...
        col.operator("texture_exporter.export_textures", text="选择目录并导出", icon='EXPORT')
        
        # 快速导出按钮（如果有上次目录）
        if prefs.export_directory:
            col.operator("texture_exporter.quick_export", text="快速导出到上次目录", icon='FILE_REFRESH')
            
            # 显示当前目录
            box = layout.box()
            box.label(text="当前导出目录:", icon='FOLDER_REDIRECT')
        ...
        if prefs.export_history:
            layout.separator()
            box = layout.box()
            box.label(text="导出历史:", icon='HISTORY')
            ...
            for i, item in enumerate(prefs.export_history):
                ...
                op.index = i
                # 删除按钮
                op_remove = row.operator("texture_exporter.remove_history", text="", icon='X')
                op_remove.index = i
